...
//需要定义突发计数器
//地址寄存器,通过突发后,加上地址偏移
//状态机里将每次需要突发的次数以及突发长度赋值给对应的寄存器

/*
状态机状态划分:
(1)启动
(2)查看[30:0]的前20位,如果是为0,转到下一个状态,如果是不为0,启动单次突发长度为256,个数为[23:0]的突发.
	 检测到突发完成done信号后,进入下一个状态.
(3)最后8位,从第一位开始判断,判断每一位是否有效.
	 有效,启动单次突发,无效,则判断下一位.
	 检测单次突发done信号,完成进入下一个状态
	 都判断结束后,发出总done信号
*/

/*
1.修改启动脉冲,一个启动脉冲是连续突发状态机,另外一个是总控制状态机.
2.修改长度寄存器判断,判断总个数而不是某一个位
*/
