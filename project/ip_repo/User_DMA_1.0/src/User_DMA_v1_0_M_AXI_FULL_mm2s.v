...
//�?要定义突发计数器
//地址寄存�?,通过突发�?,加上地址偏移
//状�?�机里将每次�?要突发的次数以及突发长度赋�?�给对应的寄存器

/*
状�?�机状�?�划�?:
(1)启动
(2)查看[30:0]的前20�?,如果是为0,转到下一个状�?,如果是不�?0,启动单次突发长度�?256,个数为[23:0]的突�?.
	 �?测到突发完成done信号�?,进入下一个状�?.
(3)�?�?8�?,从第�?位开始判�?,判断每一位是否有�?.
	 有效,启动单次突发,无效,则判断下�?�?.
	 �?测单次突发done信号,完成进入下一个状�?
	 都判断结束后,发出总done信号
*/

/*
1.修改启动脉冲,�?个启动脉冲是连续突发状�?�机,另外�?个是总控制状态机.
2.修改长度寄存器判�?,判断总个数�?�不是某�?个位
*/
